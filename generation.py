...
def split_and_predict(
    x, model, split=(8, 8, 1), overlap=1 / 3, cutoff=5, batch_size=24
):
...
def main():
    ...
    parser.add_argument(
        "path",
        type=str,
        help="Path to the file you want to perform translation on. "
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Number of tiles to split the imagery into.",
    )
    parser.add_argument(
        "--batch_size",
        type=int,
        default=16,
        help="Number of batches to be used in the prediction " "process.",
    )
    parser.add_argument(
        "--model",
        type=str,
        default="/model/model_MAE_depth_4_per_level_3.h5",
    ...
    if args.header is None:
        ...
        img = envi.open(base_path + "/" + args.header, base_path + "/" + args.path)
        img = img.load()
